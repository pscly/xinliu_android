...
| --- | --- |
| 文档日期 | 2026-07-14 |
| 当前状态 | 已整合既有 ADR 与已批准视觉选择，等待用户书面审阅 |
...
| 当前授权 | 只批准记录和审阅设计，不授权修改代码、Gradle、资源、Manifest、版本、路由或发布配置 |
| 下一道关口 | 本规格提交后，仍须由用户书面审阅并明确批准，主代理才可进入 implementation plan；规格提交本身不构成实施授权 |
...
本文关闭了 Navigation 3、Settings 架构、平台模块、首页视觉和代表能力页的设计分叉。它没有授权实施。主代理完成规格自审并提交此文件后，必须把本文交给用户书面审阅；只有用户明确批准这份书面规格，才能调用 writing-plans 进入 implementation plan。未获得书面批准前，不得修改代码、Gradle、资源、Manifest、版本或发布配置。
