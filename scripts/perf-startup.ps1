...
try {
  $adb = Resolve-Adb
  if (-not $adb) {
    Write-Output 'perf-startup.ps1: SKIP（未找到 adb；请确保 ANDROID_HOME/platform-tools 在 PATH，或存在 android-sdk/platform-tools/adb.exe）'
  } else {
    $devices = Get-OnlineAdbDevices -Adb $adb
    if (-not $devices -or $devices.Count -lt 1) {
      Write-Output 'perf-startup.ps1: SKIP（未检测到 device 状态的设备；请执行 adb devices 确认设备已授权且为 device）'
    } else {
      Write-Output ("perf-startup.ps1: devices=" + ($devices -join ', '))

      Write-Output 'perf-startup.ps1: 降噪提醒（建议手动完成后再跑，避免数据抖动）：'
      ...
      $benchmarkClass = 'cc.pscly.onememos.macrobenchmark.StartupBenchmark'
      Write-Output "perf-startup.ps1: run Macrobenchmark (cold start; StartupTimingMetric; StartupMode=COLD) class=$benchmarkClass"
      Write-Output 'perf-startup.ps1: 建议命令（等价）：'
      ...
      Write-Output 'perf-startup.ps1: OK'

      $knownDirs = Get-KnownResultDirs
      Write-Output 'perf-startup.ps1: 结果位置（不同 AGP/UTP 可能略有差异，以下为常见目录）：'
      ...
      # 可选归档：仅当 perf-results/startup 已存在时才尝试复制。
      $archiveRoot = Join-Path $projectDir 'perf-results\startup'
      if (Test-Path -Path $archiveRoot) {
        ...
        if ($copiedAny) {
          Write-Output ("perf-startup.ps1: 结果已归档到：" + $archiveDir)
        } else {
          Write-Output ("perf-startup.ps1: 未找到可归档的结果目录；如已生成请手动检查 macrobenchmark/build/outputs。归档目标：" + $archiveDir)
        }
      } else {
        Write-Output ("perf-startup.ps1: 未检测到归档目录（跳过复制）：" + $archiveRoot)
      }
    }
  }
} catch {
  $exitCode = if ($LASTEXITCODE -ne 0) { $LASTEXITCODE } else { 1 }
  Write-Output "perf-startup.ps1: FAIL (exit=$exitCode) $($_.Exception.Message)"
} finally {
  Pop-Location
}
