...
- **Flow Backend**：你们要对接的中间认证后端（示例：`https://me1.pscly.cc`，以实际配置为准）。
- **Memos Server**：真正存储数据的服务器（示例：`https://me.pscly.cc`）。
...
- 建议 App 配置：
  - `FLOW_BACKEND_BASE_URL`（例如 `https://me1.pscly.cc`）
...
- `password`：至少 6 位；建议限制为 **最多 72 字节（UTF-8）**。为避免“字节数”问题，建议 App 侧提示用户使用英文/数字/常见符号作为密码。
...
- `memos_server_url`：`https://me.pscly.cc`
...
- 校验：
  ...
  - 密码 >= 6
  - 密码与确认密码一致
  - 建议：密码 UTF-8 字节数 <= 72（否则提示用户换成英文/数字）
- 点击注册：
...
- 已在后端修复：当前 Memos（`me.pscly.cc`）管理员无权为其他用户直接创建 token，后端会改为“以用户身份创建 token”。
