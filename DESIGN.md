...
- 无统一校验错误 / 加载原语外观。
- **M6 悬浮速记层接入**（`QuickCaptureOverlayService`）：`ScrollTextField` + `InkShape.Card`（圆角 14dp）+ `colorScheme.scrim` 遮罩 + `InkCard`，形成纸墨悬浮输入体验。输入框高度 `OverlayInputMinHeight`–`OverlayInputMaxHeight`，IME 抬升上限 `OverlayImeLiftMax`。
...
| 裸 dp / 令牌收敛 | 悬浮层 / 壳层 / feature 模块 | **已核销（M6）** | M6 令牌收敛波：悬浮层（`ScrollTextField` 等）、壳层（`QuickCaptureOverlayService` 纸墨原语）、feature 模块（home/editor/todo/collections/quickcapture/sharecard/profile/auth/welcome/settings）全部收敛至 `InkSpacing`/`InkShape`/`InkBorder`；唯一残留为 `0.dp`（规则禁止令牌化 0/1.dp）。初始裸 dp 数量：home 69→19、editor 18→7、settings 116→1 等，最终生产源码（`src/main/`）裸 `[1-9][0-9]*.dp` 零匹配。 |
