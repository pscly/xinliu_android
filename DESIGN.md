...
**触控与选中语义（2026-07-21 UI 债务收口已落地）**：

- 可交互 `TagChip` 与全部 `InkChip` 采用**双层**：外层语义/点击容器 ≥ `InkSpacing.TouchTargetMin`（48dp），内层 `Surface` 保持紧凑视觉（填充/描边色块不得撑成 48dp 巨型胶囊）。
- 可点击/可选 Chip 暴露 `Selected` + `stateDescription`；静态未选 TagChip 不伪装按钮、不朗读「未选中」。
- unit：`SettingsPrimitivesAccessibilityTest`（可点击 TagChip / InkChip 选中、未选、禁用 48dp + Selected）；Roborazzi：`PaperInkComponentsScreenshotTest` light/dark/fontScale=2.0 矩阵含 Chip 行。
...
**UI 债务收口状态范围（产品已按裁决落地）**：

- **已迁移**：Editor 同步失败横幅（`SyncStatus.FAILED` + `lastSyncError`）→ 等价 `InkRetryBanner`（文案前缀「同步失败：」、动作「重试同步」、polite live region、单次 `retrySync`）。
...
**生产接线（2026-07-21 UI 债务收口已落地）**：

- 业务屏：8 个 `TopAppBar`、5 个非悬浮层 `ModalBottomSheet`、Home `SnackbarHost` 已改用对应 `PaperInk*`。
- **禁止改动且已豁免**：`QuickCaptureOverlayService` 内 raw Dialog/BottomSheet（1.15.2 已验收悬浮速记）。
- `AlertDialog` 形状/颜色继续走全局主题；显式 `PaperInkAlertDialog` 包装路径另有截图守护。
- 契约扫描：`app/src/main` + `feature/*/src/main` + `core/designsystem/src/main`；共同豁免 `PaperInkComponents.kt`；ModalBottomSheet 额外豁免 overlay 服务。守卫测试：`PaperInkProductionWiringTest`。
...
- **宿主**：`AppNavigationHost` 以 `SharedTransitionLayout` 包裹 `NavDisplay`；`ReducedMotion` 时 `sharedTransitionScope` 与 `LocalMemoSharedTransitionScope` 同为 `null`；页面级 fade / 共享轴已接线。
- **业务配对（2026-07-21 已落地）**：
  1. `core:navigation`：`LocalMemoSharedTransitionScope` + `Modifier.memoSharedBounds(uuid)`；key = `memo/<uuid>`；null/blank UUID 与 null scope early return，禁止条件调用 `rememberSharedContentState`。
  2. Home **仅** `HomeScreenMode.ACTIVE` 已有 memo 在 `MemoItem` 外层挂 bounds；Editor 内容根用路由 `memoUuid`（非 `uiState.uuid`）配对。
  3. 新建、归档列表、跨顶层分区、进程恢复不参与。
  4. 不引入 Navigation 2 或社区 `SharedEntryInSceneNavEntryDecorator`。
  5. 守卫：`MemoSharedTransitionWiringTest`；降级条件仍见 ADR 0012。
...
| 债务 | 原位置 | 核销结论 | 依据 / 保留理由 |
| --- | --- | --- | --- |
| 紧凑控件不足 `48.dp` | `SealIconButton` 默认 `44.dp`；Chip 无最小高度 | **已核销** | 印章：外层 ≥48dp + 视觉 44dp。Chip（2026-07-21）：可交互 `TagChip`/`InkChip` 外层 `minimumInteractiveComponentSize` + `defaultMinSize(TouchTargetMin)`，内层 `Surface` 保持紧凑视觉；unit + Roborazzi 守护。 |
...
| 字体放大适配未形成契约 | 固定印章、单行 Chip、表格 | **部分核销（1.16.2）** | designsystem Roborazzi 矩阵含 Seal/Chip × light/dark × fontScale=2；表格与文楷印章人工项仍保留。 |
| TalkBack 遍历与动态播报未系统化 | 卡片 / Chip / 盖章 / 弹层 | **部分核销（1.16.2）** | 状态原语 live region；`MemoItemTalkBack`；设置矩阵。**1.16.2** 为 Editor 归档/恢复、Home 批量归档、Todo 删除清单/任务确认补焦点恢复；全路径系统化仍开放。 |
...
### 8.3 UI 债务收口跟踪表（2026-07-21，事实更新 2026-07-24）

来源计划：`.omo/plans/2026-07-21-ui-debt-closeout.md`。产品 Todo 与稳定版 **1.16.0 / 1.16.1** 已发布；**1.16.2** 为纸墨对话框统一 + 关键确认焦点恢复 + Seal 大字体矩阵补强。

| 债务 | 现状 | 目标 | 计划 Todo | 状态 |
| --- | --- | --- | --- | --- |
| Chip 48dp 外触控 + 选中语义 | 双层 48dp + Selected/stateDescription；视觉紧凑 | 同左 | 2, 3 | **已落地**（unit + Roborazzi） |
| PaperInk 生产接线 | 8 TopAppBar + 5 非 overlay Sheet + 1 Home Snackbar；业务 `PaperInkAlertDialog`（1.16.2）；overlay Dialog 仍豁免 | 同左 | 4–7, 9 + 1.16.2 | **已落地** |
| Editor 同步失败横幅 | `InkRetryBanner` | 同左 | 5 | **已落地** |
| Collections/Profile/Todo 空态 | 各自布局 | **保持**，不迁 `InkEmpty` | 5, 6 | 设计冻结（不改） |
| Shared bounds 配对 | Home ACTIVE memo ↔ Editor 内容根；null scope no-op | 同左 | 1, 8 | **已落地** |
| Roborazzi 矩阵 | PaperInk/Chip/Seal 明暗+fontScale=2；feature 屏矩阵（1.16.1） | 同左 | 3, 10 + 1.16.2 | **已落地** |
| 宏基准 fail-fast | resource-id + 硬断言 | 同左 | 11 | **已落地** |
| 稳定版 1.16.0–1.16.1 | GitHub latest 已发布 | 固定签名 + Tag + Release | 14–18 | **已发布** |
| 1.16.2 纸墨/a11y 收口 | 业务 AlertDialog→PaperInk；关键确认焦点恢复；Seal 入矩阵；Profile 热力图 today 可注入 | 1.16.2 (170) 正式发布 | 本波 | **已发布** |
...
- 可点击 `InkCard`：最小高度兜底。
- **Chip（2026-07-21）**：可交互 `TagChip`/`InkChip` 外层 ≥48dp，内层紧凑视觉；`Selected`/`stateDescription` 已暴露。见 §5.4 / §8.3。
