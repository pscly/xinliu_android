...
### 8.3 UI 债务收口跟踪表（2026-07-21）

来源计划：`.omo/plans/2026-07-21-ui-debt-closeout.md`（Momus/Oracle 双审通过）。**产品 Todo 1–12 已在 worktree 落地**；真机 QA / 稳定版发布（Todo 13–18）仍待闭环，不得宣称设备 PASS 或 1.16.0 已发布。

| 债务 | 现状 | 目标 | 计划 Todo | 状态 |
| --- | --- | --- | --- | --- |
| Chip 48dp 外触控 + 选中语义 | 双层 48dp + Selected/stateDescription；视觉紧凑 | 同左 | 2, 3 | **已落地**（unit + Roborazzi） |
| PaperInk 生产接线 | 8 TopAppBar + 5 非 overlay Sheet + 1 Home Snackbar；overlay 豁免 | 同左 | 4–7, 9 | **已落地**（`PaperInkProductionWiringTest`） |
| Editor 同步失败横幅 | `InkRetryBanner` | 同左 | 5 | **已落地** |
| Collections/Profile/Todo 空态 | 各自布局 | **保持**，不迁 `InkEmpty` | 5, 6 | 设计冻结（不改） |
| Shared bounds 配对 | Home ACTIVE memo ↔ Editor 内容根；null scope no-op | 同左 | 1, 8 | **已落地**（`MemoSharedTransitionWiringTest`） |
| Roborazzi 矩阵 | designsystem PaperInk/Chip 明暗+fontScale=2；home MemoItem 三态 | 同左 | 3, 10 | **已落地**（record/verify 绿） |
| 宏基准 fail-fast | resource-id + 硬断言；无 memo 时 fail | 同左 | 11 | **已落地**（assemble 绿；真机基准待 Todo 15） |
